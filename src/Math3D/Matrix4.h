...
public:
    ...
    */
	...
	static Matrix4 MakeProjectionMatrix(float fovy, float aspect, float near, float far);
	static Matrix4 MakeViewMatrix(const Vector3 & eye, const Vector3 & at, const Vector3 & up);
	static Matrix4 MakeOrtho2DMatrix(float left, float right, float bottom, float top);
	static Matrix4 MakeBiasMatrix();
	static Matrix4 FromEuler(float x, float y, float z);
...
inline Matrix4::Matrix4(const Matrix3& m3x3)
{
    operator=(IDENTITY);
    operator=(m3x3);
}
