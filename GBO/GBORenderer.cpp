...
TGBORenderer::TGBORenderer(TGBOModel* model)
{
    gboModel = model;
...
TGBORenderer::~TGBORenderer()
{
    for(int m=0;m<(int)gboModel->meshes.size();++m)
    {
...
void TGBORenderer::PrepareBuffers(TGBOModel* model)
{
    ...
    for(int m=0;m<(int)model->meshes.size();++m)
    {
        ...
        {
            const Vector3& bpp = mesh->vertices[j].bindPosePosition;
            const Vector3& normal = mesh->vertices[j].normal;
            const Vector3& tangent = mesh->vertices[j].tangent;
            ...
            *(tptr++) = tangent.z;
            ...
            for(int v=0;v<mesh->vertices[j].weightCount&&v<4;++v)
            {
                int index = v + mesh->vertices[j].weightStart;
                ws[v] = mesh->weights[index].weight;
                bi[v] = mesh->weights[index].jointIndex;
            }
            for(int v=0;v<4;++v)
            {
                *weightPointer ++ = ws[v];
        ...
        for(int j=0;j<vertexCount;++j)
        {
            *(ptr++) = mesh->vertices[j].u;
            *(ptr++) = mesh->vertices[j].v; //REMARK: invert the v
        }
        ...
        int indexCount = 3 * (int)mesh->triangles.size();
        unsigned int* indices = new unsigned int[indexCount];
        
        unsigned int* p = indices;
        for(int j=0;j<(int)mesh->triangles.size();++j)
        {
            *(p++) = (unsigned int)mesh->triangles[j].v0;
            *(p++) = (unsigned int)mesh->triangles[j].v1;
            *(p++) = (unsigned int)mesh->triangles[j].v2;
        }
...
void TGBORenderer::SetProjection(float fovy, float aspect, float near, float far)
{
    nearPlane = near;
    farPlane = far;
...
void TGBORenderer::LookAt(const Vector3& eye, const Vector3& at, const Vector3& up)
{
    viewMatrix = Matrix4::MakeViewMatrix(eye, at, up);
    UpdateMatrix();
}

void TGBORenderer::SetWorldTransform(const Matrix4 &transform)
{
    worldMatrix = transform;
    UpdateMatrix();
}

void TGBORenderer::SetLightPosition(const Vector3 &pos)
{
...
void TGBORenderer::SetEyePosition(const Vector3& pos)
{
    glUseProgram(shaderProgram);
    eyePos = pos;
...
void TGBORenderer::UpdateMatrix()
{
    glUseProgram(shaderProgram);
...
void TGBORenderer::BindTexture(unsigned int _texture)
{
    if( texture != _texture )
    {
        texture = _texture;
...
void TGBORenderer::BindNormalTexture(unsigned int _texture)
{
...
void TGBORenderer::Render()
{
    ...
    int count = gboModel->GetJointCount();
    float matrices[count * 16];
    memset(matrices, 0, sizeof(float)*count*16);
    float* ptr = matrices;
    for(int j=0;j<count;++j)
    {
        TGBOJoint* joint = gboModel->GetJoint(j);
        
        
        Matrix4 transform = joint->worldTransform * joint->inverseBindPose;
    ...
    for(int m=0;m<gboModel->meshes.size();++m)
    {
        if( gboModel->meshes[m]->IsVisible() )
        {
            TGBOMesh* mesh = gboModel->meshes[m];
            glBindVertexArray(vertexArrayObject[m]);
            glEnableVertexAttribArray(0);
            glEnableVertexAttribArray(1);
            glEnableVertexAttribArray(2);
